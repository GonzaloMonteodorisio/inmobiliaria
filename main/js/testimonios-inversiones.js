...
window.addEventListener('DOMContentLoaded', ()=> {
    ...
    const TESTIMONIOS = []
    function traerTestimonios(url, contenedor){
        fetch(url)
        .then(response => response.json())
        .then((testimonios)=> {
            console.log(testimonios)
            TESTOMONIOS = testimonios
            crearCardTestimonio(TESTOMONIOS[0], contenedor);       
        });
    }
    traerTestimonios('https://raw.githubusercontent.com/GonzaloMonteodorisio/inmobiliaria-cyc-json/main/json/testimonios.json', contenedor_testimonios);
